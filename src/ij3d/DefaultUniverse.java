...
public abstract class DefaultUniverse extends SimpleUniverse implements
	...
	/**
	 * Reference to the InteractiveViewPlatformTransformer. This handles mouse
	 * and keyboard input which transforms the whole view. It is used by the
	 * InteractiveBehavior.
	 */
...
//	public abstract Content getSelected();
	...
	public DefaultUniverse(int width, int height/* , UIAdapter uia */) {
		...
		attributesMask.set(ATTRIBUTE_COORD_SYSTEM, false);
	...
	public void getGlobalTransform(GlobalTransform transform) {
		MultiTransformGroup group = getViewingPlatform()
	...
	/**
	 * Returns a reference to the Picker which is used by this universe to
	 * handle mouse input which aims to pick Contents and change landmark sets.
	 */
	...
	/**
	 * Returns a reference to the InteractiveViewPlatformTransformer which is
	 * used by this universe to handle mouse and keyboard input which aims to
	 * transform the viewing platform.
	 */
	 ...
	 * 
	 * @param attribute
	 *            , one of ATTRIBUTE_SCALEBAR or ATTRIBUTE_COORD_SYSTEM
	 * @param flag
	 *            , indicating whether it should be displayed or hided.
	 */
	 ...
	 * when off-screen 3D rendering is not available. You should be careful
	 * about using this - it will, for example, cause problems for scripted use
	 * of the viewer from macros if off-screen 3D rendering is not available.
	 */
	...
	/**
	 * @author Mengmeng Returns a snapshot of the given size.
	 */
	public BufferedImage takeSnapshot(int w, int h) {
		// System.out.println("Sequence takeSnapshot(int w, int h)") ;
		// return null ;
